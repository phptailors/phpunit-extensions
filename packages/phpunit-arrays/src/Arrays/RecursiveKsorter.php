 ...
 */
final class RecursiveKsorter implements RecursiveKsorterInterface
{
    /**
    ...
    {
        $array = [];
        $selector = $this->selection->getKsorter();
    ...
    {
        if ($selection->getKsorter()->supports($actual)) {
            return (new RecursiveKsorter($selection))->select($actual);
        }
