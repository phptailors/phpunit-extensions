...
//        ];
    ...
    public function testEnterLeave(array $ctor, array $calls, $result): void
    {
        ...
        if ($expect instanceof ResultInterface) {
            $expect = (new RecursiveResultUnwrapper())->unwrap($expect);
        }

        $actual = $visitor->result();
        if ($actual instanceof ActualArrayResult) {
            $actual = (new RecursiveResultUnwrapper())->unwrap($actual);
        }
...
////        $stack = [new RecursiveResultFactoryStackItem([], '', new SubjectResultCouple(null, []))];
////
